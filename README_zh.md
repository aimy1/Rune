...
Rune 是一款现代化、轻量级、全键盘驱动的 **TUI 启动器**，专为 Linux 终端环境设计。它作为统一的命令入口，涵盖应用启动、文件搜索、Shell 命令执行、AI 对话、服务管理与剪贴板历史管理，无需离开终端即可完成一切。
...
| 图标 | 功能说明 |
| :---: | :--- |
| 🖥️ | **应用启动器** — 扫描 `.desktop` 快捷方式，以后台分离会话一键启动 GUI 应用 |
| 📁 | **模糊文件搜索** — 后台线程递归扫描目录；点击文本文件自动用绑定编辑器打开 |
| 🖼️ | **图像预览渲染** — 使用半像素字符（`▄`/`▀`）与真彩色 RGB 渲染图标与图片 |
| 🐚 | **Shell 命令执行器** — 后台静默运行或开启交互式终端执行命令 |
| 🧮 | **计算器与单位换算** — 实时解析数学表达式；支持质量、温度、长度双向换算（精确至小数点后 4 位） |
| 🔑 | **SSH 快速连接** — 解析 `~/.ssh/config`，即时发起 SSH 会话 |
| 📋 | **剪贴板历史管理** — 基于 Systemd 的守护进程后台捕获剪贴历史，支持搜索并一键还原 |
| 🐳 | **Docker & Systemd 控制** — 实时查看容器（含日志预览）与系统服务状态，支持一键启动/重启 |
| 🤖 | **内置 AI 助手** — 实时 LLM 对话；支持 OpenAI、Gemini API 与本地 Ollama 服务 |
| 🔌 | **脚本插件扩展** — 将任意可执行脚本（Python、Bash、Node、Go）放入插件目录即可扩展功能 |
...
| 📝 | **文本编辑器绑定** — 绑定 `nano`、`vim`、`nvim`、`hx` 或任意自定义编辑器路径 |
| 🔌 | **插件独立开关** — 对 11 个内置插件逐一启用/禁用，顶栏标签实时重建 |
| 🤖 | **AI 提供商选择** — 切换 `openai`、`gemini`、`ollama`；端点与模型名称自动填充 |
| 🔑 | **参数行内编辑** — 在底栏状态行直接手写 API Key、模型名称与 Endpoint URL（自动掩码保护） |
| 📂 | **文件搜索深度** — 设置目录递归层数（2–6 层），保存时自动清空旧缓存、重建索引 |
| ℹ️ | **关于 Rune** — 查看版本号、许可证、GitHub 链接与作者署名 |
...
将任意可执行脚本或二进制文件放入 `~/.config/rune/plugins/` 目录。Rune 将当前查询词作为第一个参数（`$1`）调用它，并读取其在 `stdout` 输出的 JSON 数组：
