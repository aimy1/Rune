 ...
 **🌐 语言 / Language**
 ...
 Rune 是一款现代化、轻量级、全键盘驱动的 **TUI 启动器**，专为 Linux 终端环境设计。它作为统一的命令入口，涵盖应用启动、文件浏览、Shell 命令执行、服务管理与剪贴板历史管理，无需离开终端即可完成一切。
 ...
 按下 **`F2`** 呼出三栏/双栏文件浏览器。焦点切换循环：侧边栏 ➔ 文件列表 ➔ 当前路径栏 ➔ 搜索过滤框。
 
 ### 📱 响应式布局与实时预览
 - **三栏宽屏预览 (≥ 90 列)**：自动展开第三栏 `实时预览 (Live Preview)`，即时预览文本/代码文件高亮、终端半字符图片（PNG/JPG/GIF/WebP）、文件夹内容摘要及元数据。
 - **预览开关**：按下 `p` 或 `Alt-p` 随时展开或收起实时预览栏，自由切换三栏与双栏视图。
 - **窄屏模式 (< 55 列)**：仅显示 `名称` 和 `大小`。
 - **中等屏幕 (55–75 列)**：显示 `名称`、`大小` 和 `修改时间`。
 - **宽屏模式 (> 75 列)**：显示 `名称`、`大小`、`权限` 和 `修改时间`。
 
 ### 📊 多模式排序系统
 表头带有实时动态排序指示箭头（`▲` 升序 / `▼` 降序）：
 - 按 `s` 或 `Alt-s` 循环切换排序基准：**名称 (Name) ➔ 修改时间 (Modified) ➔ 大小 (Size) ➔ 扩展名 (Extension)**。
 - 按 `S`（Shift-S）或 `Alt-S` 快速反转排序方向（升序 ⇄ 降序）。
 
 ### ⌨️ 键盘快捷键 (文件管理器内全局可用)
 | 键位 | 功能 |
 | :--- | :--- |
 | `Tab` / `Shift-Tab` | 在各个面板之间循环切换焦点（侧边栏 ➔ 文件列表 ➔ 路径栏 ➔ 搜索框） |
 | `p` / `Alt-p` | 开启 / 关闭右侧实时文件与图片预览面板 |
 | `s` / `Alt-s` | 循环切换排序方式（名称、修改时间、文件大小、文件后缀） |
 | `S` / `Alt-S` | 切换排序方向（升序 ▲ ⇄ 降序 ▼） |
 | `y` / `Alt-y` | 复制当前选中项的**绝对路径**到系统剪贴板 |
 | `Y` | 复制当前选中项的**文件名**到系统剪贴板 |
 | `Backspace` | 返回上一级目录（或退出虚拟历史记录视图） |
 | `m` / `M` | 打开 / 关闭 12 项操作右键菜单 |
 | `Alt-c` / `Alt-x` | 复制 / 剪切当前选中的文件（被剪切的文件会被自动变暗淡化） |
 | `Alt-v` | 粘贴剪贴板中的文件（命名冲突时自动重命名并添加数值后缀） |
 | `Delete` / `Alt-d` | 安全移至回收站（遵循 FreeDesktop.org Trash 规范） |
 | `Shift-Delete` | 彻底永久删除文件/目录（带红色警示二次确认弹窗） |
 ...
 | 🔌 | **插件独立开关** — 对 13 个内置插件逐一启用/禁用（包括 `file_manager` 等），标签页实时重建 |
